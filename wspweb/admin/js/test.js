...
require(['jquery', 'bootstrap','pm','bootstrapvalidator','dropify','select2','ckeditor', 'datepickercn','raty', 'datatables'], function(){
    ...
    $(function(){
        ...
        $.ajax({
            url : '/Setting/Readme/Type/Edit?type=userTestApplyfor&timestamp=' + new Date().getTime() ,
            method : "get",
        ...
        //自动评价设置
        $.ajax({
            url: '/Setting/SettingAutoComment/Edit?type=testApplyfor&timestamp=' + new Date().getTime() ,
            success: function (data) {
        ...
        //价格设置
        $.ajax({
            url: '/TestApplyfor/UserTestApplyforConfig/Edit/1?timestamp=' + new Date().getTime(),
            success: function (data) {
                $('#priceForm input[name=objectid]').val(data.objectid);
                $('#priceForm input[name=type]').val(data.type);
                $('#priceForm textarea[name=content]').val(data.content);
            }
        })
        //价格设置验证
        var fieldsList = {
        ...
        $('#priceForm').attr('action','/TestApplyfor/UserTestApplyforConfig/Update');
        // 初始化表单验证
