...
require(['jquery', 'bootstrap', 'datatables', 'pm','select2','bootstrapvalidator','raty'], function () {
    var dToption = {
        ...
        "ajax": {
            url: '/workflow/api/processinstances/my_apply/list',
            dataSrc: "result"
        },
        "aoColumns": [{
            "sTitle": "流水号",
        ...
        "initComplete": function () {
            $('#changeTabel1').width("100%");
        },
        //加载结束
        "drawCallback": function () {}
    };
    $(function () {
        var table = $("#changeTabel1").DataTable(dToption);
        var usertable = $("#changeTabel1");
        //撤回
        var objectid;
        ...
        $("#disableModal").on('click', '#disableCfm', function() {
            $('#disableCfm').attr('disabled','true');
            $.ajax({
                type: "GET",
                url: "/workflow/api/usercallback",
                data:{
                    id:objectid
                },
                dataType: "json",
