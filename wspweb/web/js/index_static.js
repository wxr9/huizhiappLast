        ...
        success: function(data){
            ...
            if(data.result != ''){
                ...
                if(len > 0){
                    ...
                    html.push('<li class="active" data-target="#indexCarousel" data-slide-to="0"></li>')
                    for(var i=1; i<len; i++){
                        html.push('<li data-target="#indexCarousel" data-slide-to="'+i+'"></li>')
                    }
                    html.push('</ol><div class="carousel-inner">');

                    //第一张图片,添加active
                    html.push('<div class="item active"><img src="/'+list[0]+'"></div>')
                    for(var j=1; j<len; j++){
                        html.push('<div class="item"><img src="/'+list[j]+'"></div>')
                    }
                    html.push('</div>');
                    //前一张后一张按钮
                    html.push('<a class="left carousel-control" href="#indexCarousel" data-slide="prev"><img class="btn" src="/web/images/rightarrow.png"></a>' +
                        '<a class="right carousel-control" href="#indexCarousel" data-slide="next"><img class="btn" src="/web/images/leftarrow.png"></a>');
...
/*	$.ajax({
		...
		success: function(data){
            if(data.result != ''){
                ...
                var html=[];
                var first = ["<a href='"+result[0].details+"'><img src='/"+result[0].image+"' height='400'></a>"];
                $('.activityImg').append(first.join(''));
                for(var i=1;i< len;i++){
                    var title = result[i].title.length > 27 ? result[i].title.substring(0,23)+"..." : result[i].title;
                    html.push("<li class='img-item'><a href='"+result[i].details+"'><img src='/"+result[i].image+"' height='110'><span class='act-title'>"+title+"</span></a></li>")
                }
                html.push("<li class='img-item'><a href='/web/activity/activityCenter.html'><span class='img-more'></span><span>更多活动</span></a></li>")
        ...
        success: function(data){
           if(data != undefined){
               ...
               if(type == 'food'){
                   ...
                   html.push("<div class='lifeBox-item'>" +
                       "<a href='/web/lifeCenter/merchantDetail.html?objectid="+result[2].objectid+"' class='life-mid-img'><div class='life-rating'>" +
                       "<img src='/"+result[2].avar+"' width='470'><div class='raty-item'><div class='raty-title'>"+result[2].name+"</div><span>口味"+raty.taste+"</span><span>环境"+raty.env+"</span>" +
                       "<span>服务"+raty.service+"</span></div></div></a>" +
                       "</div>");
               }
